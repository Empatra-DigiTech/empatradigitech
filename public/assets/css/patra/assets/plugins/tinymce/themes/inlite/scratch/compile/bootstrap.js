(function (scope) {
    ...
    kernel.fp.array = def(
        ...
        function () {
            ...
            var forall = function (a, f) {
                ...
                for (var i = 0; i < a.length; ++i) if (fn(a[i]) !== true) return false;
                return true;
            };
        ...
        function () {
            var create = function () {
                ...
                var resolve = function (name, scope) {
                    ...
                    var r = scope;
                    for (var i = 0; i < parts.length && r !== undefined; ++i) r = r[parts[i]];
                    return r;
                };
    ...
    kernel.modulator.bolt = def(
        ...
        function (fn) {
            var create = function (loader, pather, namespace, path, idTransformer, options) {
                ...
                var get = function (id) {
                    var before =
                        options !== undefined && options.absolute === true ? path : pather(path);
                    var after =
    ...
    kernel.module.manager = def(
        [kernel.fp.array, kernel.fp.object, kernel.module.loader, kernel.module.fetcher],
        ...
        function (ar, obj, loader, fetcher) {
            var create = function (regulator, onerror) {
                ...
                var define = function (id, dependencies, definition) {
                    if (id === undefined) onerror('Define error: module id can not be undefined');
                ...
                var demand = function (id) {
                    ...
                    if (blueprints[id] === undefined) throw "module '" + id + "' is not defined";
        ...
        function (ar, obj, globalator) {
            var create = function (builtins, configuration) {
                ...
                var guard = function (type) {
                    if (data[type] === undefined) throw 'Unknown modulator type [' + type + '].';
                };
        ...
        function (ar, fn) {
            var create = function (sources) {
                ...
                var regulate = function (ids, define, require, demand, onsuccess, onerror) {
                    ...
                    else delay(unresolved, ids, define, require, demand, onsuccess, onerror);
                };
    ...
    loader.tag.script = def(
        ...
        function () {
            ...
            var onload = function (el, callback) {
                if (ie(el)) el.attachEvent('onreadystatechange', guard(callback));
                ...
                fs.exists(url, function (exists) {
                    if (exists)
                        fs.readFile(url, 'UTF-8', function (err, data) {
                            if (err) error('Error reading file [' + url + '], error [' + err + ']');
                            else success(data);
                        });
    ...
    loader.transporter.xhr = def(
        ...
        function () {
            ...
            var request = function (url, success, error) {
                ...
                else error('Transport error: browser does not support XMLHttpRequest.');
            };
    ...
    module.reader.bouncing = def(
        ...
        function (ar, error, specs) {
            ...
            var tick = function (file, cfg, done, read, acc) {
                ...
                if (accumulated.configs.length > 0) bounce(done, read, accumulated);
                else done({ sources: accumulated.sources, types: accumulated.types });
            };
